...
const MyEditor = () => {
  ...
  useEffect(() => {
    try {
      ...
      console.error("Error parsing saved editor content: ", error);
    }
  }, []);

  const detectFormattingTriggers = (text, selection) => {
    if (text.startsWith("# ") && selection.getAnchorOffset() === 2) {
      return { blockType: "header-one", triggerLength: 2 };
    } else if (text.startsWith("* ") && selection.getAnchorOffset() === 2) {
      return { blockType: "BOLD", triggerLength: 2 };
    } else if (text.startsWith("** ") && selection.getAnchorOffset() === 3) {
      return { blockType: "RED", triggerLength: 3 };
    } else if (text.startsWith("*** ") && selection.getAnchorOffset() === 4) {
      return { blockType: "UNDERLINE", triggerLength: 4 };
    }
    return null;
  };
  ...
  const handleChange = (state) => {
    ...
    const formatting = detectFormattingTriggers(text, selection);
    if (formatting) {
      const { blockType, triggerLength } = formatting;
      applyBlockStyle(state, blockKey, text, blockType, triggerLength);
    } else {
  ...
  const styleConfig = {
    "header-one": "text-3xl font-extrabold",
